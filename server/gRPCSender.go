...
type FastCommitRequestSender struct {
...
func NewFastCommitRequestSender(request *rpc.FastCommitRequest,
	dstServerId int, server *Server) *FastCommitRequestSender {
	s := &FastCommitRequestSender{
		request:     request,
		timeout:     0,
		dstServerId: dstServerId,
		server:      server,
...
func (c *FastCommitRequestSender) Send() {
	if c.server.serverId == c.dstServerId {
		op := NewFastCommitOp(c.request)
		c.server.commitScheduler.AddOperation(op)
		return
	}
	conn := c.server.connections[c.dstServerId]
	logrus.Infof("SEND Fast Commit %v (coordinator) to %v", c.request.CommitRequest.TxnId, conn.GetDstAddr())
	clientConn := conn.GetConn()
	if conn.GetPoolSize() > 0 {
		defer conn.Close(clientConn)
	}
	...
	_, err := client.FastCommit(context.Background(), c.request)
	...
	if err != nil {
		if dstServerId, handled := utils.HandleError(err); handled {
			...
			logrus.Fatalf("cannot send Fast commit prepareResult txn %v to server %v: %v",
				c.request.CommitRequest.TxnId, conn.GetDstAddr(), err)
		}
	} else {
		logrus.Infof("RECEIVE ACK Fast Commit %v from server %v", c.request.CommitRequest.TxnId, conn.GetDstAddr())
	}
}

type FastAbortRequestSender struct {
...
func NewFastAbortRequestSender(request *rpc.FastAbortRequest,
	dstServerId int, server *Server) *FastAbortRequestSender {
	s := &FastAbortRequestSender{
		request:     request,
		timeout:     0,
		dstServerId: dstServerId,
		server:      server,
...
func (a *FastAbortRequestSender) Send() {
	if a.server.serverId == a.dstServerId {
		op := NewFastAbortOp(a.request)
		a.server.commitScheduler.AddOperation(op)
		return
	}
	conn := a.server.connections[a.dstServerId]
	logrus.Infof("SEND Fast AbortRequest %v to %v",
	...
	client := rpc.NewCarouselClient(clientConn)
	_, err := client.FastAbort(context.Background(), a.request)
	if err != nil {
		if dstServerId, handled := utils.HandleError(err); handled {
			a.dstServerId = dstServerId
			a.Send()
		} else {
			logrus.Fatalf("cannot sent abort prepareResult txn %v to server %v: %v",
				a.request.AbortRequest.TxnId, conn.GetDstAddr(), err)
		}
	} else {
		logrus.Infof("RECEIVE ACK Abort %v from server %v",
			a.request.AbortRequest.TxnId, conn.GetDstAddr())
	}
}
