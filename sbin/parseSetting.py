...
def getTimeWindow(setting):
    return int(setting["tw"][:-2])
...
def sortByZipf(data):
    data.sort(key=getZipf)


def sortByTimeWindow(data):
    data.sort(key=getTimeWindow)


def sortByMode(data):
    data.sort(key=getMode)
